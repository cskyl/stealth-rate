#!/usr/bin/env python3
"""Bounded real-study browser QA for the edit-only payload-mode frontend.
...
def run(port: int, out: Path, browser: str) -> dict[str, object]:
    ...
    thread.start()
    # test=1 remains deterministic; this real study has no headphone quiz.
    ...
    receipt: dict[str, object] = {
        ...
        "headphone_routing": {"enabled": headphone_check, "test_query": "test=1", "purpose": "config-gated optional routing; no audio result is claimed"},
        "screenshots": {},
        "playback": [],
        "console_errors": [],
    }
    ...
    try:
        ...
        driver.execute_script(
            f"const v=document.querySelector('#clip'); v.src='/media/{STUDY}/qa-missing-control.mp4'; v.load();"
        )
        ...
        assert len(bundle["responses"]) == 2, "two edit-only trial responses must survive reload"
        assert all(row.get("task") == "edit" for row in bundle["responses"]), "MCQ response leaked into real study"
        driver.refresh()
        ...
        checks = {}
        for path in (f"/frontend/src/app.ts", f"/studies/{STUDY}/study.yaml", f"/studies/{STUDY}/private/key.json", f"/local/{STUDY}/sessions.jsonl", "/data/secret.txt"):
...
def main() -> int:
    ...
    parser.add_argument("--out", type=Path, default=REPO / "docs" / "qa_20260906" / "real_pilot")
    parser.add_argument("--browser", choices=("firefox", "chrome"), default="firefox")
    args = parser.parse_args()
    result = run(args.port, args.out, args.browser)
    print(json.dumps({"receipt": str(args.out / "receipt.json"), "download": result.get("download"), "playback": len(result.get("playback", [])), "console_errors": result.get("console_errors")}, indent=2))
    return 0
